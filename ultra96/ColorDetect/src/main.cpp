...
int main(int, char**)
{
    cv::Mat cv_yuyv,cv_gray,cv_grayf;
    ...
    for (;;)
    {
		...
		//cv_gray.data=(unsigned char *)imgOutput.data;
		xf2cv_ctr.stop();

		bound_ctr.start();
		cv::Rect rect = cv::boundingRect(cv_gray);
		cv::rectangle(cv_gray, rect, cv::Scalar(255,255,255),3);
		printf("rectangle center position x = %d, y = %d\n", (rect.tl().x +  rect.br().x)/2, (rect.tl().y +  rect.br().y)/2);
		bound_ctr.stop();

		imshow_ctr.start();
		cv::imshow("ColorDetectedImage", cv_gray);
		imshow_ctr.stop();

		if (cv::waitKey(5) >= 0)break;
    }
    cv::imwrite("ColorDetectedImage.jpeg", cv_gray);
